...
class MetaWorldPickPlaceEnv(BaseBenchmarkEnv):
    def __init__(self, task_name="pick-place-v3", render_mode=None):
    ...
    def get_coverage_id(self, obs):
        obs = np.asarray(obs)
        hand_pos = np.round(obs[:3], 1)
        obj_pos = np.round(obs[4:7], 1)
        return tuple(hand_pos.tolist() + obj_pos.tolist())
